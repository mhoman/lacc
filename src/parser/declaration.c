 ...
 * with the array length. This is parsed, but ignored for now.
 *
    ...
    String *name)
{
    ...
    switch (peek().token) {
    case '[':
        if (current_scope_depth(&ns_ident) == 1 && is_void(head)) {
            block = array_declarator(def, block, base, type, &cvrs);

        } else {
            block = array_declarator(def, block, base, type, NULL);
        }
        break;
    case '(':
        next();
